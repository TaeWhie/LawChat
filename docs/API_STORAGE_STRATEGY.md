...
### 3.1 법령 본문 최신화 (최신법령 반영)

기본적으로 `sync_laws.py`는 **이미 존재하는 본문 JSON이 있으면 건너뜀**이라, 한 번 받은 법령은 갱신되지 않는다.  
**국가법령정보 API 기준 최신 시행 법령**으로 덮어쓰려면:

1. **LAW_API_OC** 환경변수 설정 후:
   ```bash
   python scripts/sync_laws.py --force
   ```
   (`-f` 단축 옵션 동일)  
   → 기존 `api_data/laws/law/*.json`, `api_data/laws/admrul/*.json`이 있어도 API에서 다시 받아 덮어쓴다.

2. 동기화 후 **벡터 스토어 재구축**이 필요하다.  
   - 앱: 사이드바 "벡터 스토어 재구축" 버튼  
   - 또는: `python scripts/rebuild_vector_store.py`  
   재구축 후 챗봇/RAG가 갱신된 법령 본문을 사용한다.

### 3.2 연도(시행일) 설정

**특정 연도 시행법령**을 쓰려면 환경변수 **LAW_EFFECTIVE_YEAR**를 설정한다.

- 예: `LAW_EFFECTIVE_YEAR=2024`  
  → 법령 목록·본문을 **시행일 기준(eflaw)** API로 조회하고, `efYd=20240101~20241231` 범위로 해당 연도에 시행 중인 법령을 받는다.
- 미설정이면 **현행법령(공포일)** 기준(target=law)으로 조회한다.

사용 절차:

1. `.env` 또는 환경변수에 `LAW_EFFECTIVE_YEAR=2024` 등 설정
2. `python scripts/sync_laws.py --force` 실행 (해당 연도 시행법령으로 저장)
3. 벡터 스토어 재구축

연도만 바꿔서 다시 동기화·재구축하면 다른 연도 시행법령으로 전환할 수 있다.
