...
from rag.prompts import (
    system_issue_classification, user_issue_classification,
    system_checklist, user_checklist,
    system_conclusion, user_conclusion
)
...
def init_session():
    ...
    # 디버그 정보 (Developer Dashboard용)
    if "debug_info_routing" not in st.session_state:
        st.session_state.debug_info_routing = {}
    if "debug_info_step1" not in st.session_state:
        st.session_state.debug_info_step1 = {}
    if "debug_info_step2" not in st.session_state:
        st.session_state.debug_info_step2 = {}
    if "debug_info_step3" not in st.session_state:
        st.session_state.debug_info_step3 = {}
        
    # Prompt Override State
    if "prompt_overrides" not in st.session_state:
        st.session_state.prompt_overrides = {}


def _render_debug_info(debug_data: dict, title: str):
    """Developer Dashboard: RAG 파이프라인의 디버그 정보를 렌더링"""
    if not debug_data:
        return
        
    with st.expander(f"🛠️ 디버그 정보: {title}", expanded=False):
        for key, info in debug_data.items():
            if not isinstance(info, dict):
                continue
                
            st.markdown(f"**Step: {key}**")
            
            # 프롬프트
            if "system_prompt" in info:
                st.markdown("**System Prompt**")
                st.code(info["system_prompt"], language="markdown")
            if "user_prompt" in info:
                st.markdown("**User Prompt**")
                st.code(info["user_prompt"], language="markdown")
            ...
            with meta_cols[1]:
                st.metric("Reasoning Effort", info.get("reasoning_effort", "N/A"))
            with meta_cols[2]:
                parsed_success = info.get("parsed_success", "N/A")
                st.metric("Parsed Form", str(parsed_success))
                
            if "raw_response" in info:
                st.markdown("**Raw Response (JSON/Text)**")
                # 문자열이 JSON 형식인지 대략적으로 확인 (에러 방지용)
                try:
                    import json
                    # 문자열 길이가 길 수 있으므로 1000자 제한
                    st.json(json.loads(info["raw_response"]))
                except Exception:
                    st.text(info["raw_response"])
...
def main():
    ...
    def render_dev_console():
        ...
        st.caption("현재 단계에 해당하는 콘솔만 표시됩니다.")
        st.subheader("🛠️ Developer Console")
        st.caption("Prompt Overrides & Settings")
        
        with st.expander("Step 1: Classification Prompts", expanded=False):
            sc_val = st.text_area(
                "System Prompt", 
                value=st.session_state.prompt_overrides.get("system_issue_classification", system_issue_classification()),
                key="ov_sys_classify",
                height=150
            )
            uc_val = st.text_area(
                "User Prompt Template", 
                value=st.session_state.prompt_overrides.get("user_issue_classification", f"""\
사용자 상황: {{situation}}
...
"""),
            ...
            if st.button("Apply Step 1 Prompts"):
                st.session_state.prompt_overrides["system_issue_classification"] = sc_val
                st.session_state.prompt_overrides["user_issue_classification"] = uc_val
                st.success("Applied!")
                
        with st.expander("Step 2: Checklist Prompts", expanded=False):
            sc_val2 = st.text_area(
                "System Prompt", 
                value=st.session_state.prompt_overrides.get("system_checklist", system_checklist()),
                key="ov_sys_check",
                height=150
            )
            uc_val2 = st.text_area(
                "User Prompt Template", 
                value=st.session_state.prompt_overrides.get("user_checklist", "{issue} / {context} / {filtered_provisions} / {already_asked_text}"),
                key="ov_usr_check",
                height=200
            )
            if st.button("Apply Step 2 Prompts"):
                st.session_state.prompt_overrides["system_checklist"] = sc_val2
                st.session_state.prompt_overrides["user_checklist"] = uc_val2
                st.success("Applied!")
                
        with st.expander("Step 3: Conclusion Prompts", expanded=False):
             sc_val3 = st.text_area(
                "System Prompt", 
                value=st.session_state.prompt_overrides.get("system_conclusion", system_conclusion()),
                key="ov_sys_conc",
                height=150
             )
             uc_val3 = st.text_area(
                "User Prompt Template", 
                value=st.session_state.prompt_overrides.get("user_conclusion", "{issue} / {qa_text} / {full_context} / {related_articles_hint}"),
                key="ov_usr_conc",
                height=200
             )
             if st.button("Apply Step 3 Prompts"):
                st.session_state.prompt_overrides["system_conclusion"] = sc_val3
                st.session_state.prompt_overrides["user_conclusion"] = uc_val3
                st.success("Applied!")
        ...
        with st.expander("🔍 Session State Explorer", expanded=False):
            ...
            for key, value in st.session_state.items():
                if key.startswith("_") or "collection" in key or "debug" in key or "prompt_overrides" in key:
                     continue
    ...
    with main_col1:
            ...
            if st.button("이슈 분류하기"):
                if not situation.strip():
                    ...
                    with st.spinner("이슈 분류 중... (보통 5~15초 걸립니다)"):
                        try:
                            step1_res = step1_issue_classification(
                                situation, 
                                collection=col, 
                                prompt_overrides=st.session_state.prompt_overrides
                            )
                            # API 반환 튜플 구조: (issues, articles_by_issue, source, debug_info)
                            issues, articles_by_issue, step1_source, debug_info = step1_res
                        
                            if not issues:
                                st.error("제공된 법령 데이터에서 해당 상황에 맞는 이슈를 찾지 못했습니다. 다른 표현으로 다시 시도해 주세요.")
                                _render_debug_info(debug_info, "Step 1: Failed Classification")
                            else:
                                ...
                                st.session_state.debug_info_step1 = debug_info
        ...
        if st.session_state.step == "issues":
            ...
            _render_debug_info(st.session_state.get("debug_info_step1"), "Step 1: Issue Classification")
        ...
        if st.session_state.step == "checklist":
            ...
            if checklist is None:
                ...
                with st.spinner("체크리스트 생성 중... (보통 10~30초 걸립니다)"):
                    try:
                        ...
                        st.session_state.debug_info_step2 = step2_res.get("debug_info", {})
                        if error_msg:
            ...
            _render_debug_info(st.session_state.get("debug_info_step2"), "Step 2: Checklist Generation")
            ...
            if all_answered and checklist:
                ...
                if should_continue is None:
                    ...
                    with st.spinner("AI가 추가 질문 필요 여부를 판단 중..."):
                        ...
                        st.session_state.debug_info_step2.update(step2_res.get("debug_info", {}))
                ...
                if st.button(btn_label):
                    ...
                    with st.spinner("결론 생성 중..."):
                        res = step3_conclusion(
                            issue, full_qa, collection=col, 
                            narrow_answers=narrow_ans if narrow_ans else None,
                            prompt_overrides=st.session_state.prompt_overrides
                        )
                        ...
                        st.session_state.debug_info_step3 = res.get("debug_info", {}) if isinstance(res, dict) else {}
        ...
        if st.session_state.step == "conclusion":
            ...
            if conclusion_result is None:
                ...
                with st.spinner("결론 생성 중... (보통 15~30초 걸립니다)"):
                    try:
                        res = step3_conclusion(
                            issue, full_qa_list, collection=col, 
            ...
            _render_debug_info(st.session_state.get("debug_info_step3"), "Step 3: Conclusion Generation")
        
            # 검증 결과가 있으면 표시
            validation = conclusion_result.get("validation", {})
        
            with st.expander("🔍 RAG 검색 및 생략 정보 요약 (디버그 뷰)", expanded=False):
                st.markdown("**참조된 이슈 목록**")
                st.write(st.session_state.get("issues", []))
            
                st.markdown("**입력된 프롬프트 변수**")
                st.write({"issue": issue, "qa_list_length": len(full_qa_list), "narrow_answers": narrow_answers})
            
                if st.session_state.get("checklist_rag_results"):
                     st.markdown("**Checklist 과정에서 추출된 RAG 내용 요약**")
                     st.json([{"source": x.get("source"), "article": x.get("article")} for x in st.session_state.get("checklist_rag_results", [])])

            if validation and validation.get("has_missing_info"):
                st.warning(f"⚠️ 추가 정보가 필요할 수 있습니다: {validation.get('missing_info_details', '해당 사안 판단을 위한 요건 중 일부만 확인됨')}")
